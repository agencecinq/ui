...
| --------- | ---- | ------- | ----------- |
| `data-multiselectable` | boolean | `false` | Allow multiple panels open at once. |
| `data-hash` | boolean | `true` | Open the panel whose region `id` matches the URL hash (`#id` or `#/id`). |

Per panel:

| Attribute | Type | Default | Description |
| --------- | ---- | ------- | ----------- |
| `data-open` | boolean | - | Reflected open state. Useful for styling. |
| `data-deselect` | boolean | `false` | Allow collapsing the panel when it is already open. |

### Nested accordion

Nest a `<cinq-accordion>` inside a panel body. Each instance only binds its own
panels. Set `data-hash="false"` on nested hosts to avoid hash
collisions.
...
| ----- | ---------- | ------ | ----------- |
| `accordion:open` | Yes | `{ el, index }` | Fired before a panel opens. Cancel to abort. |
| `accordion:close` | Yes | `{ el, index }` | Fired before a panel closes. Cancel to abort. |
...
See the [interactive docs](https://agencecinq.github.io/ui/components/accordion/) for live examples.
