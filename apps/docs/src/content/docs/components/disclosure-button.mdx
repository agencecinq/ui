...
| `$button` | The trigger that started the action. |
...
const sync = (event) => {
  const { $button, open } = event.detail;
  triggers.forEach((trigger) => {
    if (trigger !== $button) trigger.setAttribute("aria-expanded", String(open));
  });
};
...
Pass `false` to skip event dispatch when your app already handles the transition:
...
The DOM still updates (`hidden`, `aria-expanded` via `update()` inside `open`/`close`).

### Confirm before close

```js
$host.addEventListener(EVENTS.DISCLOSURE_BUTTON_CLOSE, (event) => {
