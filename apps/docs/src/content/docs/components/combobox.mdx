...
The component does not position the popup — you own listbox CSS. For viewport-aware placement (flip when near the bottom edge, escape `overflow: hidden` ancestors), use [CSS Anchor Positioning](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_anchor_positioning):
...
`position: fixed` + anchor keeps the list in the top layer relative to the viewport while `flip-block` opens upward when there is not enough space below.
