...
| `input type="text"` + `role="spinbutton"` | Free typing without native number spinners. |
| `aria-label` | Compact controls with no visible label. |
| `aria-labelledby` | Name lives in a separate visible element. |
| Input only | No + / − buttons. Keyboard and programmatic API still work. |
| `formatValue` (property) | Optional formatter for `aria-valuetext` and the optional `[aria-live]` child. |
| Bounded / unbounded | `aria-valuemin` / `aria-valuemax` are each optional. |
...
notification fired **after** the value is committed to the DOM. It does not gate
the update: calling `event.preventDefault()` does not revert the value.
