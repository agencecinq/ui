...
<cinq-switch role="switch" aria-checked="false" tabindex="0">
  ...
  <div hidden>
    <input type="checkbox" name="notifications" tabindex="-1" aria-hidden="true" />
  </div>
</cinq-switch>
```
...
```html
<span id="prefs-reduced-motion">Reduced motion</span>
...
<p id="push-desc">Also sends an alert on mobile when an order ships.</p>
```
