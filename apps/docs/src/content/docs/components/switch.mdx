...
Pass `false` to update state without dispatching an event. The **Programmatic API**
playground section includes a silent sync button.

## Form submission

The hidden checkbox stays in sync with `aria-checked`. Submit the form to read values
with `FormData` (see **Form submission** in the playground).
