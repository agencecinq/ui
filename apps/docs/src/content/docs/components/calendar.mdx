...
Interactive demos below cover single date, range, preselection, future-only,
locale / week start, and an advanced `renderInner` events calendar.

<CalendarPlayground />

### Single date

Default mode. Add `deselect` to clear by clicking the selected day again.
...
Set `single="false"`. First click is the start, second is the end; hover or
keyboard focus previews the range. A third click resets.

```html
<cinq-calendar locale="fr" single="false" allow-past name="stay">
  <!-- header + table -->
</cinq-calendar>
```
...
| `single` | Single date (default) vs `single="false"` for range |
| `deselect` | Allow clearing selection in single mode |
