...
Only one drawer stays open at a time. When a drawer fires `drawer-open`, every
other open instance removes its `open` attribute. Open one panel, then open the
other from inside it — the first closes.
